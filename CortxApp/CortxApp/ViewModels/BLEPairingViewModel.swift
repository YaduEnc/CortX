...
final class BLEPairingViewModel: NSObject, ObservableObject {
    ...
    func startScanning() {
        ...
        // Broad scan + manual filter is more reliable with some ESP32 custom UUID adverts.
        ...
        requestInFlightTask = Task {
            do {
                ...
                await MainActor.run {
                    ...
                    guard let data = response.pair_token.data(using: .utf8) else {
...
extension BLEPairingViewModel: CBCentralManagerDelegate {
    ...
    func centralManager(
        ...
        let hasPairService = allServices.contains {
            $0.uuidString.caseInsensitiveCompare(AppConfig.BLE.pairServiceUUID) == .orderedSame
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        guard
            let service = peripheral.services?.first(where: {
                $0.uuid.uuidString.lowercased() == AppConfig.BLE.pairServiceUUID
            })
        else {
